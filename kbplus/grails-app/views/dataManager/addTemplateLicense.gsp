<div class="container" data-theme="licences">
	<div class="row">
		<div class="col s12">
			...
			<!--form-->
			<div class="row">
				<form name="searchLicencesNewTemplate" class="col s12">
					<div class="row modal-search-container z-depth-1">
						<div class="input-field search-main col s10">
							<input type="hidden" name="sort" value="${params.sort}">
							<input type="hidden" name="order" value="${params.order}">
							<input name="filter_licnewtemp" id="searchLicNewTemp" value="${params.filter}" placeholder="Search Licences" type="search"/>
							<label class="label-icon active" for="searchLicNewTemp"><i class="material-icons">search</i></label>
							<i class="material-icons input-close-icon close" id="clearLicenceNewTemplateSearch" search-id="searchLicNewTemp">close</i>
						</div>
						...
						<g:if test="${licenses?.size() > 0}">
							<g:each in="${licenses}" var="l">
								<div class="row">
									...
									<div class="col s2">
										<g:link controller="dataManager" action="newTemplateLicense" params="${[defaultInstShortcode:params.defaultInstShortcode,baselicense:l.id]}" class="btn-floating btn-flat waves-effect waves-light"><i class="material-icons">content_copy</i></g:link>
									</div>
								</div>
							</g:each>
						</g:if>
					...
					<div class="pagination" id="newLicenceTemplatePaginate">
						<g:if test="${licenses}" >
							<g:paginate action="addTemplateLicense" controller="dataManager" params="${params}" next="chevron_right" prev="chevron_left" max="${max}" total="${numLicenses}" />
						</g:if>
					</div>
				</form>
			</div>
...
<script type="text/javascript">
	$('form[name=searchLicencesNewTemplate]').submit(function() {
		var filter = $('input[name=filter_licnewtemp]').val();
		console.log(filter);

		$.ajax({
			type: "GET",
			url: "${createLink(controller:'dataManager', action:'addTemplateLicense', params:[defaultInstShortcode:params.defaultInstShortcode])}?filter="+ filter,
	...
	$('#clearLicenceNewTemplateSearch').click(function(){
	...
	$('div[id=newLicenceTemplatePaginate]').on('click', function(event) {
		var clicked = $(event.target).closest('#newLicenceTemplatePaginate a');
  		if (clicked.length > 0) {
  	...
  	$(document).ready(function(){
  		var next_pag_obj = $('div[id=newLicenceTemplatePaginate] a[class=nextLink]');
   		var prev_pag_obj = $('div[id=newLicenceTemplatePaginate] a[class=prevLink]');
