...
Subscriptions {
	subscriptions.each {subInstance->
		Subscription(id:subInstance.id) {
		    ...
		    if(subInstance.owner){Licence(id:subInstance.owner?.id) {
		    	LicenceReference subInstance.owner?.reference
		    	NoticePeriod subInstance.owner?.noticePeriod
		    	LicenceURL subInstance.owner?.licenseUrl
		    	LicensorRef subInstance.owner?.licensorRef
		    	LicenseeRef subInstance.owner?.licenseeRef
			    subInstance.owner?.orgLinks.each {orgRole->
    				RelatedOrg(id:orgRole.org.id) {
     					OrgName orgRole.org.name
     					OrgRole orgRole.roleType.value
     					OrgIDs {
		    	...
		    	LicenceProperties {
    				subInstance.owner?.customProperties.each {custprops ->
