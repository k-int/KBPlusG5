...
<g:if test="${editable}">
<g:form url="[controller: 'ajax', action: 'addCustomPropertyValue']" method="post" name="cust_prop_add_value"
...
<table id="custom_props_table" class="table table-bordered">
    ...
    <g:each in="${ownobj.customProperties}" var="prop">
        ...
        <td>
            ...
            <g:link controller="ajax" action="delCustomProperty" 
                before="if(!confirm('Delete the property ${prop.type.name}?')) return false"
                params='[propclass: prop.getClass(),ownerId:"${ownobj.id}",ownerClass:"${ownobj.class.name}", editable:"${editable}"]' id="${prop.id}"
                onComplete="runCustomPropsJS('${createLink(controller:'ajax', action:'lookup')}')" update="custom_props_div">Delete</g:link>
            </g:if>
        </td>
        </tr>
    </g:each></tbody>
...
<div id="cust_prop_add_modal" class="modal hide">
    ...
    <g:form id="create_cust_prop" name="modal_create_cust_prop"
        ...
        <div class="modal-footer">
            <button id="new_cust_prop_add_btn" class="btn btn-primary" type="submit">Add Property <i class="material-icons">add_circle_outline</i></button>
