...
<head>
	<parameter name="pagetitle" value="Create subscription worksheet" />
	<parameter name="pagestyle" value="subscriptions" />
	<parameter name="actionrow" value="subscription-generation" />
...
<body class="subscriptions">
	...
	<div class="row">
		<div class="col s12">
			<div class="mobile-collapsible-header" data-collapsible="gen-sub-list-collapsible">Search <i class="material-icons">expand_more</i></div>
			<div class="search-section z-depth-1 mobile-collapsible-body" id="gen-sub-list-collapsible">
				<g:form action="generateImportWorksheet" method="get" params="${params}">
					...
					<div class="col s12 l1">
						<g:link controller="subscriptionImport" action="generateImportWorksheet" params="${params.defaultInstShortcode?[defaultInstShortcode:params.defaultInstShortcode]:[]}" class="resetsearch">Reset</g:link>
					</div>
	...
	<div class="row">
		...
		<div class="col s12 page-response">
			...
			<g:link controller="subscriptionImport" action="generateImportWorksheet" params="${params+[clearBasket:'yes']}" class="waves-effect btn right">Clear Basket <i class="material-icons right">shopping_basket</i></g:link>
		</div>
	...
	<div class="row">
		<div class="col s12 l9">
						...
						<g:if test="${hits}">
							<g:each in="${hits}" var="hit">
								...
								<g:else>
									...
									<td><g:link controller="packageDetails" action="show" id="${hit.getSourceAsMap().dbId}" params="${params.defaultInstShortcode?[defaultInstShortcode:params.defaultInstShortcode]:[]}">${hit.getSourceAsMap().name}</g:link></td>
									<td>${hit.getSourceAsMap().consortiaName}</td>
									<td>
										<div class="actions-container">
											<g:link controller="subscriptionImport" action="generateImportWorksheet" params="${params+[addBtn:hit.getSourceAsMap().dbId]}" class="btn-floating btn-flat"><i class="material-icons">shopping_basket</i></g:link>
										</div>
									</td>
								</tr>
							</g:each>
			...
			<div class="pagination">
				<g:if test="${hits}" >
					<g:paginate controller="subscriptionImport" action="generateImportWorksheet" params="${params}" next="chevron_right" prev="chevron_left" maxsteps="10" total="${resultsTotal}"/>
				</g:if>
			</div>
		</div>
