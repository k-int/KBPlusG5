...
class IssueEntitlement implements Comparable {
  ...
  Date coreStatusEnd
  RefdataValue coreStatus // core Status is really core Medium.. dont ask. I think the concept is "What is the medium of our core entitlement"
  RefdataValue medium
...
'coverageNote','coverageDepth','embargo','startVolume','startIssue','startDate','endDate','endIssue','endVolume']
  ...
  static def refdataFind(params) {
    ...
    if ( params.q ) {
      ...
      hqlString += 'lower(ie.tipp.title.title) like ?'
      hqlParams.add("%${params.q?.toLowerCase()}%");
    }

    hqlString += "order by ie.tipp.title.title"

    def results = IssueEntitlement.executeQuery(hqlString,hqlParams)

    results?.each { t ->
      def ie = com.k_int.ClassUtils.deproxy(t)
      result.add([id:"${ie.class.name}:${ie.id}",text:ie.tipp.title.title])
    }
