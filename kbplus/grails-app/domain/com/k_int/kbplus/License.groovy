...
class License implements Comparable<License>{
  ...
  // Ian - changing this - it's misleading :: static hasOne = [onixplLicense: OnixplLicense]
  ...
  static mapping = {
	  ...
	  jiscLicenseId column: 'lic_jisc_id'
  ...
  static constraints = {
    ...
    jiscLicenseId(nullable: true, blank: true)
  ...
  boolean isEditableBy(user, request) {
    hasPerm("edit", user);
  }

  boolean hasPerm(perm, user) {
    log.debug("hasPerm(${perm},${user})");
    boolean result = false

    if ( perm=='view' && this.isPublic?.value=='Yes' ) {
      log.debug("License is public and perm is view");
      result = true;
    }
    ...
    log.debug("hasPerm returns ${result}");
    result;
  }
  ...
  def checkPermissions(perm, user) {
    ...
    log.debug("The list of principles : ${principles} who grant ${perm} for this user (${user.id})");
    ...
    orgLinks.each { ol ->
      ...
      log.debug("checkPermissions navigating org_link(${ol.id}) between license ${this.id} and org ${ol.org.id} - role type is ${ol.roleType}");
      ...
      if ( perm_exists ) {
        log.debug("Org link (${ol.id}) between License ${this.id} and org ${ol.org}(${ol.org.name}) grants perm ${perm}")
    ...
    log.debug("Result of checkPermissions(${perm},${user}) : ${result}");
    result
  }
...
//      log.debug("MAP TYPE ${oldMap[crp]?.class} OLD MAP: ${oldMap[crp]} NEW MAP: ${newMap[crp]}")
  ...
  def notifyDependencies(changeDocument) {
    ...
    def derived_licenses = License.executeQuery('select l from License as l where exists ( select link from Link as link where link.toLic=l and link.fromLic=:fl )',[fl:this])
  ...
  */
  ...
  static def refdataFind(params) {
      ...
      def result = []
      def  ql = License.executeQuery("select l ${INSTITUTIONAL_LICENSES_QUERY}".toString(),[orgId:params.inst?.toLong(),orgRole:params.roleType?.toLong(),publicS:params.isPublic?.toLong(),ref:"${params.q.toLowerCase()}"])
      if ( ql ) {
          ql.each { lic ->
