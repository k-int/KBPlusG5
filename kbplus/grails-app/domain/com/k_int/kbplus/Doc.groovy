...
class Doc {
  ...
  static mapping = {
       ...
       blobContent column:'doc_blob_content'
        blobLength column:'doc_blob_length'
  ...
  static constraints = {
    ...
    blobLength(nullable:true, blank:false)
  }
  ...
  @Deprecated 
  /**
   *  Use DocstoreService.getStreamFromUUID(doc.uuid) instead.
   */
  def getBlobData() {
  ...
  /**
   * It's not good to have the response referenced in a domain class, the docstore service
   * now has responsibility for streaming doc responses. See the streamDoc method there.
   *
   *  Use DocstoreService.getStreamFromUUID(doc.uuid) instead.
   */
  @Deprecated 
