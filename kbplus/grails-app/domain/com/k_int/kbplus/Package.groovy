...
'''
  ...
  static mapping = {
             ...
             description column:'pkg_description'
               listPrice column:'pkg_list_price'
...
//                 orgs sort:'org.name', order: 'asc'
 ...
 @Transient
  ...
  def addToSubscription(subscription, createEntitlements) {
    ...
    // Step 2 - Connect
    def new_pkg_sub = SubscriptionPackage.findBySubscriptionAndPkg(subscription, this) ?: new SubscriptionPackage(subscription:subscription, pkg:this).save(flush:true, failOnError:true);
    ...
    if ( createEntitlements ) {
      ...
      tipps.each { tipp ->
        if(tipp.status?.value != "Deleted"){
          ...
          // See if there is already an IE for this title
          def current_ie = IssueEntitlement.findBySubscriptionAndTipp(subscription,tipp)

          if ( ( current_ie == null ) || ( current_ie.status?.value=='Deleted' ) ) {
            ...
            def new_ie = new IssueEntitlement(status: live_issue_entitlement,
                                              ...
                                              coverageNote:tipp.coverageNote).save(flush:true, failOnError:true)      
          }
        }
      }
    }
  ...
  @Transient
  def addToProduct(product) {
	  def new_pkg_prd = PackageProduct.findByProductAndPkg(product, this) ?: new PackageProduct(product:product, pkg:this).save(flush:true, failOnError:true);
  }
  ...
  **/
  ...
  static def refdataFind(params) {
    ...
    queryResults?.each { t ->
      def p = com.k_int.ClassUtils.deproxy(t)
      def resultText = p.name
      def date = p.startDate? " (${sdf.format(p.startDate)})" :""
      resultText = params.inclPkgStartDate == "true" ? resultText + date : resultText
      resultText = params.hideIdent == "true" ? resultText : resultText+" (${p.identifier})"
      result.add([id:"${p.class.name}:${p.id}",text:resultText])
    }    
  ...
  public long getTitleCount() {
    long result=TitleInstancePackagePlatform.executeQuery(TITLE_COUNT_QRY,[pkg:this.id]).get(0);
    result;
  }
}
