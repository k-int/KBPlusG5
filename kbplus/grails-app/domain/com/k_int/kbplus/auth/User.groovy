...
class User {
  ...
  static constraints = {
    ...
    recAdminEmails blank:false, nullable:true
  ...
  def hasSystemRole(rolename) {
    def result = false
    def role = com.k_int.kbplus.auth.Role.findByAuthority(rolename)
    if (role) {
      def userrole = com.k_int.kbplus.auth.UserRole.findByUserAndRole(this, role)
      if (userrole) {
        result = true
      }
    }
