...
class GlobalSourceSyncService {
  ...
  def runAllActiveSyncTasks() {
    ...
    if ( running == false ) {
      def future = executorService.submit({ internalRunAllActiveSyncTasks() } as java.lang.Runnable)
    }
    else {
  ...
  def private doOAISync(sync_job) {
    log.debug("doOAISync");
    if ( parallel_jobs ) {
      def future = executorService.submit({ intOAI(sync_job.id) } as java.lang.Runnable )
    }
    else {
      intOAI(sync_job.id)
    }
