...
class ChangeNotificationService {
  ...
  def broadcastEvent(contextObjectOID, changeDetailDocument) {
    ...
    if (contextObject.metaClass.respondsTo(contextObject, 'getStatus')) {
      if ( contextObject.getStatus().value == 'Deleted' ) {
        is_deleted = true;
      }
    }

    // If context object has a status other than deleted, OR context object has no status
    if ( contextObject && !is_deleted ) {
      ...
      def new_queue_item = new ChangeNotificationQueueItem(oid:contextObjectOID, 
                                                           ...
                                                           ts:new Date())
      if ( new_queue_item.save(flush:true, failOnError:true) ) {
      ...
      log.debug("Not issuing a pending change for a deleted item");
    }
      ...
      pendingOIDChanges.each { poidc ->
        ...
        if ( contextObject != null ) {
          if ( contextObject.metaClass.respondsTo(contextObject, 'getURL') ) {
            pc_title= "<p>Changes on <a href=\"${contextObject.getURL()}\">${contextObject.toString()}</a> ${new Date().toString()}</p>";
          }
          else  {
            pc_title = "<p>Changes on ${contextObject.toString()} ${new Date().toString()}</p>"
          }
          sw.write("</p><p><ul>");
        }
        else {
            ...
            contextObject.notificationEndpoints.each { ne ->
              ...
              switch ( ne.service ) {
                case 'announcements':
                  ...
                  def newAnnouncement = new Doc(title: ( pc_title ?: 'Automated Announcement' ),
                                                type:announcement_type,
                                                content:announcement_content,
                                                dateCreated:new Date(),
                                                user:User.findByUsername('admin')).save(flush:true, failOnError:true);
        ...
        pc_delete_list.each { pc ->
          ...
          pc.delete(flush:true, failOnError:true)
        }
      }
    }
    catch ( Exception e ) {
  ...
  def registerPendingChange(prop, target, desc, objowner, changeMap ) {
    ...
    if ( new_pending_change.save(flush:true, failOnError:true) ) {
    }
    else {
