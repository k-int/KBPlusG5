...
class PendingChangeService {
  ...
  def performAccept(change,httpRequest) {
    log.debug("performAccept(${change},...)");
      def result = true
      PendingChange.withNewTransaction { TransactionStatus status ->
        ...
        try {
          ...
          switch ( parsed_change_info.changeType ) {
            ...
            case 'PropertyChange' :  // Generic property change
              if ( ( parsed_change_info.changeTarget != null ) && ( parsed_change_info.changeTarget.length() > 0 ) ) {
                ...
                log.warn("Unable to resolve target of pending change ${parsed_change_info.changeTarget} ${parsed_change_info}");
              }
              break;
            case 'TIPPEdit':
          ...
          log.debug("Process pending changes and remove from target object");
    ...
    def processCustomPropertyChange(parsed_change_info){
      def changeDoc = parsed_change_info.changeDoc
       if ( ( parsed_change_info.changeTarget != null ) && 
            ( parsed_change_info.changeTarget.length() > 0 ) ) {
          ...
          if(updateProp){
            switch (changeDoc.event){
              ...
              case "CustomProperty.updated":
                log.debug("Update custom property ${updateProp.type.name} ${changeDoc}")
                if(changeDoc.type == 'com.k_int.kbplus.RefdataValue' ){
                  ...
                  updateProp."${changeDoc.prop}" = updateProp.parseValue("${changeDoc.new}", changeDoc.type)
                }
