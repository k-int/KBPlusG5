...
class InstitutionsService {
    ...
    def copyLicence(params){
        ...
        def org = Org.findByShortcode(params.defaultInstShortcode)
        ...
        if ( lic_name.length() > 255 ) {
          lic_name = lic_name.substring(0,255);
        }
	...
	def copyTemplate(params){
		def baseLicense = params.baselicense ? License.get(params.baselicense) : null;
		def org = Org.findByShortcode(params.defaultInstShortcode)

		def license_type = RefdataCategory.lookupOrCreate('License Type', 'Template')
		def license_status = RefdataCategory.lookupOrCreate('License Status', 'Current')
		def lic_name = params.lic_name?:"Copy of ${baseLicense?.reference}"
		def licenseInstance = new License(reference: lic_name,
				status: license_status,
		...
		// the url will set the shortcode of the organisation that this license should be linked with.
		if (!licenseInstance.save(flush: true)) {
			log.error("Problem saving template license ${licenseInstance.errors}");
			return licenseInstance
		} else {
			log.debug("Save ok");
			def licensee_role = RefdataCategory.lookupOrCreate('Organisational Role', 'Licensee')
    ...
    **/
    def generateComparisonMap(unionList, mapA, mapB, offset, toIndex, rules){
      ...
      for (unionTitle in unionList){
		...
		//new background colours used for when comparing subs and packages for KB+7. Kb+6 original above
		if(delete && comparison == -1 ) value = [objA,null, "danger", "#F2DEDF"];
		else if(update && comparison == 1) value = [objA,objB, "warning", "#FDF8E4"];
		else if(insert && comparison == null) value = [null, objB, "success", "#DEF0D8"];
		else if (noChange && comparison == 0) value = [objA,objB, "", ""];
