...
class JasperReportsController {
        ...
        if(fileName.endsWith(".jrxml") || fileName.endsWith(".jasper")){
          ...
          if(existingReport == null){
            ...
            if(newReport.hasErrors()){
              flash.error += message(code: 'jasper.upload.saveError', args: [fileName])
            }else{
            ...
            if(existingReport.hasErrors()){
              ...
              flash.error += message(code: 'jasper.upload.overwrite', args: [fileName])
            }
          } 
        }else{
          flash.error += message(code:'jasper.upload.wrongFormat',args:[fileName])
        }
      }
  ...
  def generateReport(){
    ...
    log.debug("generateReport...${params}");
    ...
    filteredDateParams.each { key, value ->
      def stringVal = value
      def newVal = sdf2.format(sdf2.parse(stringVal))+" 00:00:00"
