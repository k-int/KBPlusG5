...
class SubscriptionImportController {
  ...
  def generateImportWorksheet() { 
	...
	//TODO
	//rewrite this as we are not using checkboxes anymore, using select dropdown boxes and these are separated out into the individual sections
    ...
    def inst = request.getAttribute('institution')//params.id ? Org.get(params.id) : null
  ...
  @Secured(['ROLE_USER', 'IS_AUTHENTICATED_FULLY'])
  def importSubscriptionWorksheetModal() {
	  def result = [:]
	  result
  }
  ...
  def processSubscriptionImport() {
    ...
    if ( params.ecount != null ) {
	  ...
	  log.debug("earliest date: ${earliest_start_date}")
	  log.debug("latest date: ${latest_end_date}")
  
      new_subscription.startDate = earliest_start_date
      new_subscription.endDate = latest_end_date
      new_subscription.save(flush:true, failOnError:true)
